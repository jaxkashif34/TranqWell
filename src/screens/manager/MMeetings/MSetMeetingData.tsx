...
export const MSetMeetingData: FC<
  MAuthScreenProps<MMeetingNavigatorParamsList, 'MSetMeetingData'>
> = ({ navigation }) => {
  const [isFormSubmitting, setFormSubmitting] = useState(false);
  const formik = useFormik({
    initialValues: initialValues('MEETING_FIELDS'),
    validationSchema: createMeetingValidation,
    onSubmit,
  });

  async function onSubmit(
    values: FieldsForForm<'MEETING_FIELDS'>,
    action: FormikHelpers<FieldsForForm<'MEETING_FIELDS'>>
  ) {
    setFormSubmitting(true);

    navigation.navigate('MSetMeetingUsers', { data: values });
