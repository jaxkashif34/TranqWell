...
import type { MAuthScreenProps, MMeetingNavigatorParamsList } from "~types";
...
export const MMeetingDetails: FC<
  MAuthScreenProps<MMeetingNavigatorParamsList, "MMeetingDetails">
> = ({ navigation, route }) => {
  const meeting = route.params.meeting;
  const { manager } = useAppSelector(selectManagerState);
  const handleNavigateToJoinCallScreen = () => {
    return navigation.navigate("JoinCallScreen", {
      meeting,
      user: manager,
    });
  };
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <SView className="flex-1">
        ...
        <SView className="px-4 flex-1 justify-end py-2">
          <SButton variant="customer" onPress={handleNavigateToJoinCallScreen}>
