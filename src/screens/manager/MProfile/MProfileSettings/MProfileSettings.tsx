...
import type { MAuthScreenProps, MProfileNavigatorParamsList } from '~types';
import { useAppDispatch } from '~hooks';
import { logOutManager, resetMState, resetRole } from '~redux';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Ionicons, Feather } from '@expo/vector-icons';
import { SettingNavigationCard, SettingToggleCard } from '~components';

export const MProfileSettings: FC<
  MAuthScreenProps<MProfileNavigatorParamsList, 'MProfileSettings'>
> = ({ navigation }) => {
  const [visible, setModalVisible] = React.useState(false);
  const dispatch = useAppDispatch();
  const handleLogout = async () => {
    dispatch(resetMState())
      ...
      <SView className="px-4 py-4 flex-1">
        <SScrollView className="pb-4">
          <SettingToggleCard
            ...
            text="Enable or disable notifications to control your alerts and updates"
            icon={<Feather name="bell" size={33} color="black" />}
            variant="manager"
          />
          <SettingNavigationCard
            heading="Change Password"
            text="Update your password anytime to enhance your account security"
            icon={<Feather name="lock" size={33} color="black" />}
            onPress={() => navigation.navigate('MChangePassword')}
            variant="manager"
          />
          <SettingNavigationCard
          ...
          <SettingNavigationCard
            ...
            onPress={() => navigation.navigate('MDeleteProfile')}
            variant="manager"
          />
        </SScrollView>
        ...
        <SButton variant="manager" onPress={() => setModalVisible(true)}>
